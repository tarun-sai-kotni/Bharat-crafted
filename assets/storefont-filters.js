...
class CollectionFiltersForm extends HTMLElement {
  ...
  connectedCallback() {
    ...
    '.m-toolbar--sortby select[name="sort_by"]'
    );
    if (sortSelect) {
    ...
    const sortSelect = document.querySelector(
      ...
      document.body.addEventListener("click", function (e) {
       const item = e.target.closest(".m-sortby-mobile--item");
        if (!item) return;
        ...
        /* Active UI */
        document
          .querySelectorAll(".m-sortby-mobile--item")
          .forEach(el => el.classList.remove("active"));
        item.classList.add("active");
        /* Close drawer */
        document
          .querySelector(".m-sortby-mobile--content")
    ...
    /* Initial bind */
    document.addEventListener("DOMContentLoaded", bindMobileSort);


    /* Re-bind after filter ajax reload */
    document.addEventListener("shopify:section:load", bindMobileSort);




    /* DOM watcher (theme rebuilds html) */
    const observer = new MutationObserver(bindMobileSort);
    ...
    const clearBtn = this.querySelector(".clear-filters-btn");
      if (clearBtn) {
        clearBtn.addEventListener("click", (e) => {
          e.preventDefault();
          this.renderPage(""); // re-renders section with no filters — form resets automatically
        });
      }
